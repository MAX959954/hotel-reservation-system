...
<template>
  <footer class="bg-ink border-t border-hairline">
    <div class="max-w-[1600px] mx-auto px-6 md:px-10 py-14 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div>
        <h3 class="text-[11px] font-medium uppercase tracking-[0.12em] text-bone-dim mb-4">{{ $t('footer.explore') }}</h3>
        <ul class="flex flex-col gap-3 text-sm font-light text-bone-dim">
          <li><RouterLink to="/hotels" class="hover:text-bone transition-colors">{{ $t('footer.stays') }}</RouterLink></li>
          <li><RouterLink to="/apartments" class="hover:text-bone transition-colors">{{ $t('footer.apartments') }}</RouterLink></li>
          <li><RouterLink to="/journal" class="hover:text-bone transition-colors">{{ $t('footer.journal') }}</RouterLink></li>
        </ul>
      </div>

      <div>
        <h3 class="text-[11px] font-medium uppercase tracking-[0.12em] text-bone-dim mb-4">{{ $t('footer.cities') }}</h3>
        <ul class="flex flex-col gap-3 text-sm font-light text-bone-dim">
      ...
      <div>
        <h3 class="text-[11px] font-medium uppercase tracking-[0.12em] text-bone-dim mb-4">{{ $t('footer.support') }}</h3>
        <ul class="flex flex-col gap-3 text-sm font-light text-bone-dim">
          <li>
            <a href="mailto:support@folio.example" class="hover:text-bone transition-colors">{{ $t('footer.contactSupport') }}</a>
          </li>
          <li><RouterLink to="/bookings" class="hover:text-bone transition-colors">{{ $t('footer.manageBooking') }}</RouterLink></li>
        </ul>
      </div>
    </div>
